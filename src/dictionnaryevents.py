...
class DictEvents:
    ...
    def look_for_arbitrage(self, bot):
        ...
        """
        for event, odds in self.dict.items():
            if ((1/odds['1'].max() + 1/odds['x'].max() + 1/odds['2'].max()) < 1):
                team1 = StandardNames.team_ids[event.id1],
                team2 = StandardNames.team_ids[event.id2],
                message = ""
                message += 'Found arbitrage opportunity (profit margin = ' + str((1 - ((1 / odds['1'].max()) + (1 / odds['x'].max()) + (1 / odds['2'].max())))*100) + '%):\n'
                message += 'event = ' + datetime.datetime.strftime(event.as_of_date, '%d/%m/%Y %H:%M') + ' : ' + team1 + ' vs ' + team2 + '\n'
                message += 'Bet ' + team1 + ' on ' + odds['1'].idxmax() + ' (odds : ' + str(odds['1'].max()) + ')\n'
                message += 'Bet draw on ' + odds['x'].idxmax() + ' (odds : ' + str(odds['x'].max()) + ')\n'
                message += 'Bet ' + team2 + ' on ' + odds['2'].idxmax() + ' (odds : ' + str(odds['2'].max()) + ')\n'
